...
- 依赖方向固定为 `apps/app -> packages/modules/* -> packages/core`；宿主只负责组合和启动，core 不依赖业务模块。

## 目录与职责
- `apps/app` 是私有薄宿主，只维护入口、manifest、模块清单、固定 bootstrap 页面和 Vite 配置。
- `packages/core` 是可发布底座，维护启动、认证、请求、状态、动态导航、基础页面、公共静态资源和构建插件。
- `packages/modules/system` 是可发布业务模块，维护个人中心、资料、设置和 AI 的 API、RPC 与页面。
- `packages/cli` 是可发布脚手架，负责创建同结构的独立 app workspace。
...
- `apps/app/src/module-manifest.ts` 是宿主唯一模块清单，顺序决定静态页面和 `viewKey` 的覆盖优先级。
...
- 业务模块只能通过 `@liujitcn/kratos-app-core` 的公开 exports 复用底座，不相对引用其他包源码。
...
- app RPC 统一执行 `pnpm generate:rpc` 或 `make -C backend ts-app`，分别生成 core 与 system 的 `src/rpc`。
