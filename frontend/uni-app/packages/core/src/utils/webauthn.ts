...
/** 将 PublicKeyCredential 序列化为服务端协议使用的 JSON。 */
...
export async function createWebAuthnCredential(optionsJson: string): Promise<string> {
  if (typeof navigator === 'undefined' || !navigator.credentials?.create) {
    throw new Error(t('core.login.mfa_webauthn_setup_unsupported'))
  }
  const credential = await navigator.credentials.create({
    publicKey: parseCreationOptions(optionsJson),
  })
  if (!(credential instanceof PublicKeyCredential))
    throw new Error(t('core.login.mfa_webauthn_setup_invalid'))
...
export async function getWebAuthnAssertion(optionsJson: string): Promise<string> {
  if (typeof navigator === 'undefined' || !navigator.credentials?.get) {
    throw new Error(t('core.login.mfa_webauthn_login_unsupported'))
  }
  const credential = await navigator.credentials.get({
    publicKey: parseRequestOptions(optionsJson),
  })
  if (!(credential instanceof PublicKeyCredential))
    throw new Error(t('core.login.mfa_webauthn_login_invalid'))
  return serializeCredential(credential)
}
