...
H5 构建默认输出到 `backend/data/app` 并使用 `/app/` 公共路径。微信小程序开发和生产产物统一输出到 `apps/taro-app/dist/mp-weixin`，微信开发者工具导入该目录即可。开发时 H5 和微信小程序可以同时运行，页面装配由 runner 共享，构建产物分别写入 `dist/h5` 和 `dist/mp-weixin`。
