...
| 文件 | 用途 | 使用位置 |
| --- | --- | --- |
| `zh-CN.json` | System 模块简体中文固定 UI 默认语言包，也是七语键集合和占位符校验基准。 | Taro 固定文案默认语言及缺失翻译回退。 |
...
| `ko-KR.json` | System 模块韩语语言包。 | Taro 切换为韩语时使用。 |
| `fr-FR.json` | System 模块法语语言包。 | Taro 切换为法语时使用。 |
| `es-ES.json` | System 模块西班牙语语言包。 | Taro 切换为西班牙语时使用。 |
