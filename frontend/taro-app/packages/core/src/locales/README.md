...
本目录提供 Taro 应用底座和公共页面使用的稳定文案，包括登录、启动状态、导航、认证、MFA/WebAuthn、文件处理和网络错误提示。语言包由 `../index.ts` 的 `coreModule` 导入并注册，应用启动时由 `bootstrapKratosTaroApp` 调用 `registerLocaleMessages` 合并；页面、hooks、请求工具通过 `useI18n()` 或 `t()` 使用。
