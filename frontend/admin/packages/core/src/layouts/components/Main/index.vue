...
function createComponentWrapper(component, route) {
  ...
  const wrapperCache = wrapperMap.get(wrapperName);
  if (!wrapperCache || wrapperCache.component !== component) {
    // 按完整路由生成独立包装组件，避免多标签切换时复用同一个动态组件实例导致内容串页。
    const wrapper = { name: wrapperName, render: () => h(component) };
    wrapperMap.set(wrapperName, { component, wrapper });
    return wrapper;
  }
  return wrapperCache.wrapper;
}
