...
| 文件 | 用途 | 使用位置 |
| --- | --- | --- |
| `zh-CN.json` | System 模块简体中文默认语言包，也是三语键集合和占位符校验基准。 | 管理端默认语言及缺失翻译回退。 |
...
语言包由 `module.ts` 传入 `systemAdminModule.messages`，由 core 的 `registerLocaleMessages` 校验三语键集合、命名空间和占位符后注册到 Vue I18n；业务代码统一使用 `t('system....')` 或 `useLocaleStore()` 读取。
