...
import {
  normalizeCodeGenLocaleConfigMap,
  serializeCodeGenLocaleConfigMap,
  type CodeGenLocaleConfigRecord
} from "../../../../utils/code_gen_i18n";

const CODE_GEN_DATABASE_TABLE_URL = "/v1/admin/code-gen/database/table";
const CODE_GEN_TABLE_URL = "/v1/admin/code-gen/table";

/** 代码生成字段的 REST JSON 请求体。 */
type CodeGenColumnPayload = Omit<CodeGenColumn, "i18n_config"> & {
  i18n_config: CodeGenLocaleConfigRecord;
};

/** 保存代码生成字段的 REST JSON 请求体。 */
type SaveCodeGenColumnPayload = Omit<SaveCodeGenColumnRequest, "code_gen_columns"> & {
  code_gen_columns: CodeGenColumnPayload[];
};
...
export class CodeGenColumnServiceImpl implements CodeGenColumnService {
  ...
  SaveCodeGenColumn(request: SaveCodeGenColumnRequest): Promise<Empty> {
    return service<SaveCodeGenColumnPayload, Empty>({
      url: `${CODE_GEN_TABLE_URL}/${request.table_id}/column`,
      method: "put",
      data: {
        ...request,
