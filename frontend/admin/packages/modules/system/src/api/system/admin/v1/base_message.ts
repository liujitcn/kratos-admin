...
export class BaseMessageServiceImpl implements BaseMessageService {
  ...
  PageBaseMessage(request: PageBaseMessageRequest): Promise<PageBaseMessageResponse> {
    return service<PageBaseMessageRequest, PageBaseMessageResponse>({ url: MESSAGE_URL, method: "get", params: request });
  }

  /** 查询消息详情。 */
  GetBaseMessage(request: GetBaseMessageRequest): Promise<BaseMessageDetail> {
    return service<GetBaseMessageRequest, BaseMessageDetail>({ url: `${MESSAGE_URL}/${request.id}`, method: "get" });
  }

  /** 创建消息草稿。 */
  CreateBaseMessage(request: CreateBaseMessageRequest): Promise<CreateBaseMessageResponse> {
    return service<BaseMessageForm | undefined, CreateBaseMessageResponse>({
      url: MESSAGE_URL,
      method: "post",
      data: request.base_message
    });
  }

  /** 更新消息草稿。 */
  UpdateBaseMessage(request: UpdateBaseMessageRequest): Promise<Empty> {
    return service<BaseMessageForm | undefined, Empty>({
      url: `${MESSAGE_URL}/${request.base_message?.id ?? ""}`,
      method: "put",
      data: request.base_message
    });
  }
  ...
  DeleteBaseMessage(request: DeleteBaseMessageRequest): Promise<Empty> {
    return service<DeleteBaseMessageRequest, Empty>({ url: `${MESSAGE_URL}/${request.id}`, method: "delete" });
  }

  /** 发布消息。 */
  PublishBaseMessage(request: PublishBaseMessageRequest): Promise<Empty> {
    return service<PublishBaseMessageRequest, Empty>({
      url: `${MESSAGE_URL}/${request.id}/publish`,
      method: "put",
      data: request
    });
  }
  ...
  CancelBaseMessageSchedule(request: CancelBaseMessageScheduleRequest): Promise<Empty> {
    return service<CancelBaseMessageScheduleRequest, Empty>({
      url: `${MESSAGE_URL}/${request.id}/schedule/cancel`,
      method: "put",
      data: request
    });
  }
  ...
  RevokeBaseMessage(request: RevokeBaseMessageRequest): Promise<Empty> {
    return service<RevokeBaseMessageRequest, Empty>({ url: `${MESSAGE_URL}/${request.id}/revoke`, method: "put", data: request });
  }

  /** 重试消息投递任务。 */
  RetryBaseMessageDispatch(request: RetryBaseMessageDispatchRequest): Promise<Empty> {
    return service<RetryBaseMessageDispatchRequest, Empty>({
      url: `${MESSAGE_URL}/dispatch/${request.id}/retry`,
      method: "put",
      data: request
    });
  }
