...
`internal/agent` 是当前项目对 CloudWeGo Eino 的适配层。业务包不直接堆 Eino 细节，而是通过这里按 Quick Start 的组件路线使用 ChatModel、Agent/Runner、Tool、Middleware、Callback 和 Workflow。

这个目录的目标有三个：

1. 收拢 Eino 原生 import，避免 `internal/biz/base/ai`、`internal/server` 等业务包直接感知第三方框架细节。
...
| 9 | 上层调用方 | `internal/biz/base/ai` | 基础 AI 运行时如何只保留协议、提示词、会话和响应结构，把 Eino 能力交给本目录。 |

## 主链路

### AI 助手对话链路

1. `internal/biz/base/ai.Runtime` 负责读取会话、历史消息、附件、工具开关和前端 SSE 协议。
...
1. `internal/biz/base/ai` 通过 `workflow.Registry` 管理当前启用模块提供的固定流程。
...
1. 新增平行适配目录，例如 `internal/agent/{new_framework}`。
...
- `internal/biz/base/ai` 负责 AI 助手业务运行时、会话消息、工具候选策略、SSE 和前端需要的响应结构。
- 业务模块只负责输入、提示词和响应结构，不直接承担模型结构化输出细节。
- `internal/server` 负责注册生成工具并交给运行时，不承担 Agent 编排。
- `internal/agent` 负责 Eino 适配能力，不写具体业务提示词和业务决策。
- Eino 原生 import 应尽量只出现在 `internal/agent` 和 `api/gen/*_agent_tool.go` 生成产物中。业务包如需模型、工具、结构化输出或固定流程能力，应通过本目录的项目内门面调用。
