...
func NewCronServer(baseJobRepo *data.BaseJobRepository, registry *coreTask.Registry) *CronServer {
	return &CronServer{
...
func (c *CronServer) lookupTaskExec(invokeTarget string) (coreTask.TaskExec, error) {
	// 调用目标为空时，无法定位实际任务实现。
	if invokeTarget == "" {
