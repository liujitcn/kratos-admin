...
func (c *BaseDictItemCase) PageBaseDictItem(ctx context.Context, req *adminv1.PageBaseDictItemRequest) (*adminv1.PageBaseDictItemResponse, error) {
	...
	if req.GetLabel() != "" {
		...
		translatedIDs, err = c.baseTranslationCase.GetTargetIdsByName(ctx, adminv1.TranslationTargetType_TRANSLATION_TARGET_TYPE_BASE_DICT_ITEM, req.GetLabel())
		if err != nil {
			return nil, err
		}
	...
	translations, err = c.baseTranslationCase.GetBaseTranslationMapByTargetType(ctx, adminv1.TranslationTargetType_TRANSLATION_TARGET_TYPE_BASE_DICT_ITEM, targetIds)
	if err != nil {
		return nil, err
	}
	for _, item := range list {
...
func (c *BaseDictItemCase) GetBaseDictItem(ctx context.Context, id int64) (*adminv1.BaseDictItemForm, error) {
	...
	var translations map[int64][]*adminv1.BaseI18n
	translations, err = c.baseTranslationCase.GetBaseTranslationMapByTargetType(ctx, adminv1.TranslationTargetType_TRANSLATION_TARGET_TYPE_BASE_DICT_ITEM, []int64{id})
	if err != nil {
		return nil, err
	}
...
func (c *BaseDictItemCase) DeleteBaseDictItem(ctx context.Context, id string) error {
	...
	return c.tx.Transaction(ctx, func(ctx context.Context) error {
		...
		return c.baseTranslationCase.DeleteBaseTranslation(ctx, adminv1.TranslationTargetType_TRANSLATION_TARGET_TYPE_BASE_DICT_ITEM, ids)
	})
}
...
func (c *BaseDictItemCase) saveBaseTranslation(ctx context.Context, req *adminv1.BaseDictItemForm, entity *models.BaseDictItem) error {
	return c.baseTranslationCase.SaveBaseTranslation(ctx, adminv1.TranslationTargetType_TRANSLATION_TARGET_TYPE_BASE_DICT_ITEM, entity.ID, entity.Label, req.GetTranslations(), func(ctx context.Context, label string) error {
