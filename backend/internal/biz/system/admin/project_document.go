...
func (c *ProjectDocumentCase) TreeProjectDocument(ctx context.Context, _ *adminv1.TreeProjectDocumentRequest) (*adminv1.TreeProjectDocumentResponse, error) {
	coreProjects := c.docs.Projects(ctx)
	projects := make([]*adminv1.ProjectDocumentProject, 0, len(coreProjects))
	for _, project := range coreProjects {
		projects = append(projects, c.mapProjectDocumentProject(ctx, project))
	}
	return &adminv1.TreeProjectDocumentResponse{Projects: projects}, nil
}

// GetProjectDocument 查询项目文档详情。
func (c *ProjectDocumentCase) GetProjectDocument(ctx context.Context, req *adminv1.GetProjectDocumentRequest) (*adminv1.ProjectDocument, error) {
	document, exists := c.docs.Get(ctx, req.GetId())
	if !exists {
		return nil, errorsx.ResourceNotFound("项目文档不存在")
	}
	return c.mapProjectDocument(ctx, document), nil
}

// mapProjectDocumentProject 将 Core 项目目录转换为 Admin 接口项目目录。
func (c *ProjectDocumentCase) mapProjectDocumentProject(ctx context.Context, project dto.Project) *adminv1.ProjectDocumentProject {
	item := &adminv1.ProjectDocumentProject{
		Key:       project.Key,
		Name:      c.localizeProjectName(ctx, project.Name),
...
func (c *ProjectDocumentCase) mapProjectDocument(ctx context.Context, document dto.Document) *adminv1.ProjectDocument {
	return &adminv1.ProjectDocument{
		Id:          document.ID,
		ProjectKey:  document.ProjectKey,
		ProjectName: c.localizeProjectName(ctx, document.ProjectName),
		Path:        document.Path,
		Name:        document.Name,
...
func (c *ProjectDocumentCase) localizeProjectName(ctx context.Context, name string) string {
