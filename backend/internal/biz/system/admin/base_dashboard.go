...
func (c *BaseDashboardCase) GetBaseDashboardOverview(ctx context.Context) (*adminv1.BaseDashboardOverview, error) {
	tenantID, err := c.resolveDashboardTenant(ctx)
	if err != nil {
		return nil, err
	}
	userQuery := c.userRepo.Query(ctx).BaseUser
	userOpts := make([]repository.QueryOption, 0, 1)
	if tenantID > 0 {
		userOpts = append(userOpts, repository.Where(userQuery.TenantID.Eq(tenantID)))
	}
	var userCount int64
	userCount, err = c.userRepo.Count(ctx, userOpts...)
	if err != nil {
		return nil, err
	}
	roleQuery := c.roleRepo.Query(ctx).BaseRole
	roleOpts := make([]repository.QueryOption, 0, 1)
	if tenantID > 0 {
		roleOpts = append(roleOpts, repository.Where(roleQuery.TenantID.Eq(tenantID)))
	}
	var roleCount int64
	roleCount, err = c.roleRepo.Count(ctx, roleOpts...)
	if err != nil {
		return nil, err
	}
	...
	if tenantID > 0 {
		loginOpts = append(loginOpts, repository.Where(loginQuery.TenantID.Eq(tenantID)))
	}
	...
	if tenantID > 0 {
		operationOpts = append(operationOpts, repository.Where(operationQuery.TenantID.Eq(tenantID)))
	}
...
func (c *BaseDashboardCase) GetBaseDashboardLoginDistribution(ctx context.Context) (*adminv1.BaseDashboardDistributionResponse, error) {
	...
	for result, count := range counts {
		items = append(items, &adminv1.BaseDashboardDistributionItem{
			Label: adminv1.BaseAuditResult(result).String(),
			Count: count,
		})
	}
	return &adminv1.BaseDashboardDistributionResponse{Items: items}, nil
...
// resolveDashboardTenant 将普通租户统计限制在自身租户，默认租户保留平台汇总能力。
func (c *BaseDashboardCase) resolveDashboardTenant(ctx context.Context) (int64, error) {
