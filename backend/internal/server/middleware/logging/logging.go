...
import (
	...
	"github.com/liujitcn/kratos-admin/backend/internal/data/queue"
		...
		return func(ctx context.Context, req interface{}) (reply interface{}, err error) {
			...
			if info, ok := transport.FromServerContext(ctx); ok {
				...
				if htr, htrOk := info.(*http.Transport); htrOk {
					...
					if !(htr.Operation() == basev1.OperationFileServiceMultiUploadFile || htr.Operation() == basev1.OperationFileServiceUploadFile || htr.Operation() == basev1.OperationFileServiceDownloadFile) {
					...
					if htr.Operation() == basev1.OperationLoginServiceLogin {
						var loginRequest basev1.LoginRequest
