...
type BaseRedactStoragePolicy struct {
	...
	TableName_ string                `gorm:"column:table_name;type:varchar(128);not null;uniqueIndex:unique_base_redact_storage_policy,priority:1;comment:数据库表名" json:"table_name"`                    // 数据库表名
	ColumnName string                `gorm:"column:column_name;type:varchar(128);not null;uniqueIndex:unique_base_redact_storage_policy,priority:2;comment:数据库字段名" json:"column_name"`                 // 数据库字段名
	...
	DeletedAt  soft_delete.DeletedAt `gorm:"column:deleted_at;type:bigint unsigned;not null;uniqueIndex:unique_base_redact_storage_policy,priority:3;comment:删除时间;softDelete:milli" json:"deleted_at"` // 删除时间
}
