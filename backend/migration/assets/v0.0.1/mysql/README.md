...
- 菜单与权限：预置首页、租户管理、用户管理、系统管理四个管理端根菜单，以及移动端根菜单 `99000000`。系统管理下包含数据备份（数据归档、数据备份及各自的配置/记录/恢复记录）、配置管理和开发工具；移动端首页为 `99010000`，我的页面为 `99090000`；开发工具包含运维监控、API 文档、日志查询、缓存查询和代码生成。菜单的 `api` 字段记录页面实际调用的受保护服务方法，角色通过菜单编号获得权限。
- 开发工具已迁入系统管理，使用 `91980000` 作为目录，子菜单按 SQL 顺序使用 `91980100` 至 `91980540`；代码生成按钮使用 `91980501` 至 `91980509`，隐藏页面使用 `91980510` 至 `91980540`。
- 安全能力：登录管理菜单及其服务、按钮权限只授予平台超级管理员，默认不创建登录策略记录；数据脱敏菜单及默认手机号响应掩码授予系统管理员；OAuth 客户端管理菜单、多因素认证配置和认证方式字典直接写入初始化数据。相关业务表由数据层自动迁移创建。
- 脱敏规则：`base_redact_rule.up.sql` 初始化代码固定支持的全部模板，以及按数据源保存的用户手机号、邮箱、证件号入库策略和按服务名保存的用户列表、分页、详情接口出库策略；管理端通过“数据脱敏”下的脱敏规则、入库脱敏和出库脱敏页面维护。
...
个人信息菜单授予本人登录记录查询权限，服务端固定当前用户与租户；在线用户菜单及下线权限仅供平台超级管理员使用。
