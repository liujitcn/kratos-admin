...
- 配置与字典：包含管理端和应用端配置、验证码、OAuth 自动注册、租户编号显示、多因素认证策略和认证方式；日志入库回退使用隐藏系统配置 `auditLogSpool`，日志保留策略写入 `base_table_archive`，数据库备份策略写入 `base_table_backup`，会话生命周期和上传扫描由 `authn.session`、`oss.upload_security` 配置；持久化字典覆盖菜单、审计、登录策略、消息、权限和代码生成等枚举。
