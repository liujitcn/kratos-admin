...
- メニューと権限: Admin 側にはホーム、テナント管理、ユーザー管理、システム管理のルートを用意します。モバイルのルートは `99000000`、ホームは `99010000`、マイページは `99090000` です。システム管理には、データアーカイブ、データバックアップ、および各設定/記録/復元ページを含むデータバックアップと、システム設定・実行設定を含む設定管理があります。システム管理下の開発ツールには運用監視、API ドキュメント、ログ照会、キャッシュ照会、コード生成を含みます。各メニューの `api` には実際に呼び出す保護対象サービスメソッドだけを記録し、ロールはメニュー ID で権限を取得します。
- 開発ツールの同階層メニューは SQL 順に `91980100` から `91980540`、`sort` は `1` から `9` を使用します。コード生成ボタンは `91980501` から `91980509`、親 ID は `91980500` です。
...
- 脱敏ルール: `base_redact_rule.up.sql` はコードが固定対応する全テンプレート、データソース単位で管理するユーザーの電話番号・メール・身分証番号の保存ポリシー、およびサービス名単位で管理するユーザー一覧・ページング・詳細操作の出力ポリシーを初期化します。Admin ではデータマスキング配下のルール、入庫脱敏、出庫脱敏画面から管理できます。
