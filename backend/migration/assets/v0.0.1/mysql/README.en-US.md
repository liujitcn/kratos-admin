...
The profile menu grants access to the current user’s login history; the server fixes both user and tenant from the authenticated identity. Online session listing and revocation are available to the platform super administrator and tenant administrators, while tenant administrators are restricted to their own tenant.
