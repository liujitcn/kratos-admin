...
- Menus and permissions: the Admin roots are Home, Tenant Management, User Management, and System Management. The mobile root is `99000000`, with Home at `99010000` and My Account at `99090000`. System Management contains Data Backup (data archiving, data backup, and their configuration/record/restore pages), Configuration Management, and Development Tools. Development Tools includes operations monitoring, API documentation, log query, cache query, and code generation. Each menu's `api` field lists the protected service methods it actually calls, and roles receive permissions through menu IDs.
- Development Tools menus use IDs `91980100` through `91980540` in SQL order, with `sort` values `1` through `9`. Code generation buttons use `91980501` through `91980509`, under parent `91980500`.
