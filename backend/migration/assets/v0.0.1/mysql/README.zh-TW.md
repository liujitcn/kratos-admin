...
- 設定與字典：包含管理端與應用端設定、驗證碼、OAuth 自動註冊、租戶編號顯示、多因素認證策略與方式；日誌入庫回退使用隱藏設定 `auditLogSpool`，保留策略寫入 `base_table_archive`，資料庫備份策略寫入 `base_table_backup`，工作階段生命週期和上傳掃描由 `authn.session`、`oss.upload_security` 設定。持久化字典涵蓋選單、稽核、登入策略、訊息、權限與程式碼生成等列舉值。
