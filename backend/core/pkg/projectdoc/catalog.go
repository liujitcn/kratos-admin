...
// ParseCatalog 解析扁平构建目录或 MarshalCatalog 生成的项目文档集合。
func ParseCatalog(data []byte, projectKey, projectName string) (*Catalog, error) {
	...
	for _, project := range payload.Projects {
		documents = appendProjectDocuments(documents, project.Documents, project.Key, project.Name)
...
func validateDocument(document Document) (Document, error) {
	...
	if normalizedPath == "." || normalizedPath == "" || normalizedPath == ".." || path.IsAbs(normalizedPath) || strings.HasPrefix(normalizedPath, "../") {
