...
var documentKeyPattern = regexp.MustCompile(`^[a-zA-Z0-9][a-zA-Z0-9._-]*$`)
...
func (r *Registry) Register(documents ...Document) error {
	...
	for _, document := range documents {
		...
		existing, exists := documentsByKey[document.Key]
		if exists {
			if existing.Name == document.Name && bytes.Equal(existing.Data, document.Data) {
				continue
			}
			return fmt.Errorf("OpenAPI 文档 key %q 重复", document.Key)
		}
		...
		for _, apiKey := range apiKeys {
			ownerKey, found := documentKeysByAPI[apiKey]
			if found {
				return fmt.Errorf("OpenAPI 文档 %q 与 %q 包含重复接口 %s", document.Key, ownerKey, displayAPIKey(apiKey))
			}
			documentKeysByAPI[apiKey] = document.Key
		}
...
func (r *Registry) DocumentByOperation(path, method string) (Document, bool) {
	...
	documentKey, exists := r.documentKeysByAPI[newAPIKey(path, method)]
	if !exists {
