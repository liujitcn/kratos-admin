...
本文记录 admin 与 app 当前共用的账号密码、验证码、OAuth、密码密文和 Token 机制。接口以 `backend/api/proto/base/v1/login.proto`、`oauth.proto` 为准。
...
| 应用端登录页 | `frontend/app/src/pages/login/login.vue` |
| 应用端密码加密 | `frontend/app/src/utils/passwordCrypto.ts` |
| 应用端 Token 和请求 | `frontend/app/src/utils/auth.ts`、`http.ts` |
