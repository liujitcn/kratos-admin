...
| `frontend/admin/apps/admin` | core と System を組み合わせる既定の管理端末ホスト | 非公開 `@liujitcn/kratos-admin`、`backend/web/admin` |
| `frontend/uni-app` | ホーム、ログイン、マイページ、プロフィール、設定、WebView、AI の uni-app 端末 | `@liujitcn/kratos-uni-app`、`backend/web/uni-app` |
...
flowchart LR
  ...
  Backend --> DB[("MySQL")]
  Backend --> Redis[("Redis")]
  Backend -->|"/admin/"| AdminFiles["web/admin"]
  Backend -->|"/uni-app/"| UniAppFiles["web/uni-app"]
