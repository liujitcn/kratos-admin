...
```mermaid
flowchart LR
  Models["backend/internal/data/gen/models"] --> Migrate["GORM 建表"] --> Main[("kratos_admin")]
  Default["admin 内置版本化迁移"] --> Main
  Area["admin 内置地区迁移"] --> Main
```

本地后端默认开启自动迁移，按当前模型创建或更新表结构，然后执行内置版本化 SQL。
表结构变化后，需要使用当前开发库执行 `make gorm-gen` 更新 `internal/data/gen`。共享和生产环境必须先评估迁移风险，再决定是否开启自动迁移。
