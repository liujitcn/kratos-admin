...
`frontend/admin` 是当前唯一业务前端入口，负责平台系统管理。`frontend/app` 仅作为基础应用壳子保留，不承载独立业务模块。管理后台使用 Vue 3、Vite、TypeScript、Element Plus 和 Pinia，开发期通过 Vite 代理访问后端，生产构建部署到 `backend/data/admin` 并由后端以 `/admin/` 托管。

公共组件的完整清单、属性、事件、实例方法和示例见[前端组件清单与使用说明](前端组件清单.md)。
...
flowchart LR
  ...
  SQL["backend/migration/assets/mysql/<version>/<feature>.up.sql"] --> Menu["菜单 / 按钮 / 接口权限"]
  Menu --> Router["动态路由"]
  Router --> Page
```
