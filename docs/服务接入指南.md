...
flowchart LR
  ...
  Go --> Service["backend/internal/service"]
  Service --> Server["backend/internal/server"]
...
| 协议包 | 后端目录 | 使用端 |
| --- | --- | --- |
| `base.v1`、`common.v1` | `internal/service/base` | 管理后台和应用端共用能力 |
| `system.admin.v1` | `internal/service/admin` | 管理后台 |
| `system.app.v1` | `internal/service/app` | 应用端基础能力 |
...
- `api/gen/go`
- `internal/data/gen`
- `internal/server/assets/openapi.yaml`
...
```text
backend/internal/service/<module>/admin/biz # 业务用例、事务和权限边界
backend/internal/service/<module>/admin/*_service.go
backend/internal/server/<module>/admin/register.go # HTTP、gRPC、MCP 注册
```

服务层只负责接收生成的请求、调用 biz 并转换错误；数据库访问使用
`internal/data/gen` 生成的 repository、model 和 query，不在运行时拼接 SQL。

### 4. 把模块挂到当前服务

传输层模块必须实现 `core.Module` 的三个方法：
...
var _ core.Module = Services{}

func (s Services) RegisterGRPC(grpc.ServiceRegistrar) {}
...
1. 在 `backend/internal/service/<module>/admin/init.go` 提供业务层 `ProviderSet`。
2. 在 `backend/internal/server/<module>/admin/register.go` 提供传输层 `ProviderSet`。
3. 在 `backend/app.go` 的 `moduleProviderSet` 中加入这两个 ProviderSet。
4. 在 `newModules` 参数和模块列表中加入新的 `Services`；外部模块则通过
   `WithAdditionalModules` 注入。
5. 执行 `make wire`，不要直接编辑 `backend/wire_gen.go`。

跨模块数据访问只能使用 `api/gen/go` 暴露的生成客户端。独立微服务注入标准远程 gRPC 连接；
同进程启动器使用 `kratosadmin.NewModule` 返回的 `ClientConn()` 创建完全相同的客户端。禁止对外
暴露或直接引用 `internal/data/gen`、Case 和 Service 实现。
...
`base_migration` 表记录版本，并按 `module` 区分迁移模块、按 `data_source` 区分目标数据源。
版本记录只在该版本的全部升级脚本成功后写入；任一脚本失败会回滚当前版本、记录错误并阻止服务启动，
修复脚本后重启会重新执行。只有 `backend/configs/data.yaml` 中 `enable_migrate: true` 时才会执行。
