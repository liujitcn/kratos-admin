...
api/proto
  ...
  -> internal/biz/{base/v1,system/admin/v1,system/app/v1,...}
  -> internal/data/gen
```

- Proto 是 HTTP、gRPC、校验和前端类型的唯一契约源。
- service 只做传输适配和错误兜底，业务规则放 biz，数据库访问限制在 data。
- `internal/server/{base/v1,system/admin/v1,system/app/v1}` 注册传输服务；`app.go` 负责模块门面和 Wire 组合根。
...
| `make ts-uni-app` | uni-app core 与 system 包的 `src/rpc` |
| `make ts-taro-app` | Taro core 与 system 包的 `src/rpc` |
