...
| 域 | 协议包 | 后端服务 | 前端调用 |
| --- | --- | --- | --- |
| 公共能力 | `base.v1`、`common.v1` | `internal/service/base` | `frontend/admin`、`frontend/app` |
| 管理后台 | `system.admin.v1` | `internal/service/admin` | `frontend/admin/src/api` |
| 应用端 | `system.app.v1` | `internal/service/app` | `frontend/app/src/api` |
...
Proto
 ...
 ├─ make openapi   -> backend/internal/server/assets/openapi.yaml
...
- 基础模块通过 `core.Module` 和 Backend 内部 Wire 显式装配，禁止使用隐式注册。
