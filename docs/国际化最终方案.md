...
服务运行时的机器翻译使用 `go-utils` 的 Google V1 provider，只存在于两个显式入口：
...
草稿接口默认关闭，通过 `I18N_TRANSLATION_DRAFT_ENABLED=true` 或应用 metadata 开启。服务端读取受控源文本，限制目标语言和单条 2000 字节大小，串行调用并使用 8 秒 HTTP 超时；已有 `reviewed` 译文不会被覆盖。机器结果必须人工审核后才参与运行时读取或迁移。

Google V1 是非官方、无 SLA 的草稿 provider，不能用于登录、错误响应、普通 CRUD 或任意文本翻译代理。provider 不可用时，固定语言包、已审核翻译和主语言回退仍必须正常工作。
