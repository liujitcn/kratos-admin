...
| 动态中文权威值 | 继续使用主表中文字段；翻译表只存 `en-US` 等非默认语言，不重复存 `zh-CN` |
...
| 代码生成 | 新页面、菜单、按钮和错误必须同时生成稳定 key 与三语配置，缺英语或日语时阻止正式生成 |

这不是把“国际化”委托给在线翻译。国际化的运行时能力必须完全独立于 Google V1；Google V1 仅减少第一次录入英语和日语草稿的工作量。即使该端点永久不可用，已审核的三语界面、菜单、字典和错误仍应正常工作。
...
- 管理端 core、System 模块、uni-app core/System、Taro core/System 均注册 `zh-CN`、`en-US`、`ja-JP` JSON 语言包，并校验 key 与占位符集合。
...
- 动态菜单、字典和字典项使用三张翻译表，运行时仅读取 `reviewed` 译文并回退中文主字段。
- 后端 `errorsx` 保持冻结的六类 `reason`，静态错误目录和 locale 中间件提供三语 `message`。
- 初始化数据和 `v0.0.2` 迁移保留中文主字段，并提供英语、日语审核译文及翻译状态。

## 3. 目标和非目标

### 3.1 目标

1. 管理端、uni-app 和 Taro 所有实际展示给用户的固定文案支持 `zh-CN/en-US/ja-JP`。
...
7. Google V1 能生成受保护、可追踪、不会覆盖人工译文的英语或日语草稿。
...
- `zh-CN/en-US/ja-JP` key 和占位符集合一致；
- 冲突直接失败，禁止后注册模块静默覆盖。
...
- `frontend/uni-app/packages/core` 和 `frontend/taro-app/packages/core` 各自注册 core/System 的三语 JSON 语言包。
...
- 应用端菜单仍使用后端解析后的 `meta.title`；缺失英语或日语时回退中文，不把裸 key 展示给用户。
...
英语和日语目录保留同一个 ID，只替换 `other`。动态参数使用 `{{.Field}}` 一类模板，`message_args` JSON 解码成 `TemplateData`；模板执行失败按回退链返回中文，不能把模板错误交给客户端。
...
主表字段是默认 `zh-CN` 的唯一权威值，三张翻译表只保存非默认 locale。当前只创建 `en-US`、`ja-JP` 行，不创建 `zh-CN` 行，避免默认中文在主表和翻译表之间双写。

运行时菜单、字典和普通消费者接口只读取 `reviewed` 译文；`machine/pending` 仅在管理端翻译维护接口中可见。未审核英语或日语按缺失处理并回退主表中文，机器草稿不会直接发布给最终用户。
...
覆盖目录、页面菜单、权限按钮和外链。`base_menu.meta.title` 保留为默认中文、旧客户端兼容和最终兜底；`path/name/component/api` 不翻译。
...
1. 先按现有权限读取菜单。
2. 收集菜单 ID。
3. 默认 locale 直接使用主表标题；非默认 locale 使用一次 `IN` 查询读取当前 locale 的 `reviewed` 翻译。
...
`base_dict.name` 和 `base_dict_item.label` 保留为默认中文。`code/value/tag_type` 在所有语言下不变。

`OptionBaseDict`、列表、详情和 App 字典读取沿用现有字段，只在返回前解析 `name/label`。管理表单返回翻译维护数据。按当前语言搜索时先用生成的类型化查询取得翻译匹配 ID，再与默认字段匹配结果合并、去重并查询主表；不得使用手写 SQL 或 N+1 查询。

### 8.4 管理界面

- 表单维护默认中文、英语和日语；默认中文直接写主表，不再维护重复的 `zh-CN` 翻译行。
- 内置菜单/字典要求三语完整；运行期自定义数据可以缺英语或日语并回退中文。
- `machine/reviewed/pending/源文已变化` 必须有明确状态展示。
- “生成英语/日语草稿”只在默认中文已经保存后可用。
- 人工编辑英语或日语后保存为 `reviewed`，并清空 `translation_provider` 或保留历史到审计日志；当前表只表示最新状态。
- 普通菜单、字典和 App 查询不得返回 `machine/pending` 文本；管理员确认后才进入运行时读取范围。
- Google V1 失败只提示草稿生成失败，不回滚已保存的中文业务数据。
...
| 覆盖 | 永不覆盖已有静态英语/日语或 `reviewed` 动态译文 |
...
- JSON 以 locale 为键，保存 `en-US` 和 `ja-JP`，例如 `{"en-US":{"comment":"User Management"},"ja-JP":{"comment":"ユーザー管理"}}`；
- 默认 `zh-CN` 继续读取现有 `comment` 和左树配置，不在 JSON 中重复保存；
- 其他由中文描述派生并进入页面标题、字段、菜单或错误的配置，也必须纳入对应层级的 `i18n_config`。

英语和日语配置在编辑草稿时可以为空，预览必须列出缺失项，正式生成前必须完整。

### 10.2 生成产物

生成器必须：

- 生成模块 `zh-CN/en-US/ja-JP` JSON 和稳定 key；
...
```text
backend/migration/assets/v0.0.2/mysql/i18n.up.sql
backend/migration/assets/v0.0.2/mysql/i18n.description.md
```

不得修改 `v0.0.1`。迁移应：

1. 创建三张翻译表及索引。
2. 增加代码生成表级、列级 `i18n_config` JSON 字段。
3. 不回填 `zh-CN` 翻译行，现有主表字段继续作为默认中文。
4. 为 103 条内置菜单、10 个字典和 35 个字典项提供人工复核后的 `en-US`、`ja-JP` `reviewed` 初始化数据。
5. 保留 `base_menu.meta.title/base_dict.name/base_dict_item.label`。
6. 使用 `INSERT ... SELECT ... WHERE NOT EXISTS` 或等价幂等方式，不覆盖用户已维护译文。
7. 不把运行时调用 Google V1 写进迁移；迁移中的英语和日语必须在提交前已经生成并审核。
...
5. **数据库结构**：在开发库创建三张翻译表及代码生成 `i18n_config` 字段，执行 `make gorm-gen`。
...
11. **代码生成器**：英语/日语配置、语言包/key、菜单翻译、错误 key、预览缺失检查和正式生成阻断。
12. **版本化迁移**：提交 `v0.0.2` 表结构和完整三语初始化数据，不调用在线 provider。
13. **全量检查与运行验收**：静态检查、生成、Go 测试、三个前端 workspace 的 lint/type/build，以及三种语言的关键流程。
...
1. 管理端、uni-app、Taro 每个模块 `zh-CN/en-US/ja-JP` key 集合一致。
2. 后端 `zh-CN/en-US/ja-JP` key 集合一致。
...
8. 内置菜单、字典和字典项具备完整三语迁移数据。
9. Google V1 结果的哨兵、标签、URL、代码片段完全恢复。
10. 草稿命令不修改已有英语或日语，动态草稿不覆盖 `reviewed`。
...
- 管理端、uni-app 和 Taro 登录前后都能切换 `zh-CN/en-US/ja-JP`，刷新和重新登录后保留选择。
...
- 同一账号三种语言下菜单结构、ID、路由、权限完全一致，只有标题变化。
- 同一字典三种语言下 `code/value/tag_type` 一致，只有 `name/label` 变化。
- 缺英语或日语时回退中文，不显示空白或 key。
- 查询是批量的，无 N+1。
- `machine/reviewed/pending/过期` 状态准确，自动流程永不覆盖 `reviewed`。
- Google V1 失败不影响中文记录保存、菜单读取和字典读取。

### 14.3 后端错误

- 同一错误三种语言保持相同 `code/reason/message_key`，只改变 `message`。
...
- `--write` 只补缺失项，可中断、可续跑、不覆盖人工英语或日语。
...
生成命令以仓库 Makefile 的真实目标为准；如果 Proto 改动要求 OpenAPI/TS/Wire 同步生成，应一次性执行完整项目命令。还要用 `zh-CN/en-US/ja-JP` 分别完成管理端、uni-app、Taro 的登录、菜单、典型 CRUD、字典展示、校验失败、认证失效、权限失败和无刷新切换运行时验证。
...
- **前端问题**：locale 回退 `zh-CN`，动态资源仍由中文主字段兜底。
- **后端目录问题**：错误回退原中文 message 和安全 INTERNAL_ERROR。
- **数据问题**：翻译表是附加数据，不删除主表中文字段；可以停用 locale 解析而不破坏路由/权限/字典契约。
- **切换官方 Google**：将构造替换为 go-utils V2/V3 并增加正式认证配置；保留 `translation_provider` 历史值区分来源。

不得通过删除/修改 `v0.0.2` 已执行迁移来回滚生产数据库；需要数据库回退时新增更高版本迁移。

## 17. 完成定义

只有同时满足以下条件才算国际化改造完成：

- 接口契约、后端、管理端、动态数据、代码生成器和 `v0.0.2` 迁移全部闭环。
- 所有生成产物由项目命令生成。
- 前后端 key、占位符、错误 key、中文可见文案检查通过。
- 103 条内置菜单、10 个字典、35 个字典项具有审核后的三语数据。
- `go test ./...`、前端 lint、类型检查和生产构建通过。
- 三种语言在管理端、uni-app、Taro 的运行时验收通过。
