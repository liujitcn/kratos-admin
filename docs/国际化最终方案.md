...
| 代码生成 | 新页面、菜单、按钮和错误必须同时生成稳定 key 与所有受支持语言配置，缺非主语言时阻止正式生成 |

这不是把“国际化”委托给在线翻译。国际化的运行时能力必须完全独立于 Google V1；Google V1 仅减少第一次录入非主语言草稿的工作量。即使该端点永久不可用，已审核的所有受支持语言界面、菜单、字典和错误仍应正常工作。
...
- 后端 `errorsx` 保持冻结的六类 `reason`，静态错误目录和 locale 中间件提供所有受支持语言 `message`。
...
- `frontend/uni-app/packages/core` 和 `frontend/taro-app/packages/core` 各自注册 core/System 的所有受支持语言 JSON 语言包。
...
语言包是编译期能力清单：后端错误目录、三个前端 workspace 的 core/System 目录和代码生成语言目录必须拥有相同的 BCP 47 文件集合。`scripts/sync_locales.py` 校验这些集合并生成后端 `core/pkg/locale/manifest.json`、前端注册文件和日期库映射，因此新增语言不需要修改 Go、TypeScript 或模块注册源代码。

`base_language` 是运行期部署配置，只保存语言名称、排序、启用状态和主语言标记，不作为编译期支持白名单。需要为新环境插入默认语言时，使用 `make i18n-sync I18N_MIGRATION_VERSION=vX.Y.Z` 生成版本化迁移；迁移使用 `INSERT IGNORE`，不会覆盖已有部署的启用和主语言设置。

语言名称由 `base_language.language_name/native_name` 在运行时提供。语言包中的 `common.language.*` 仅是数据库不可用时的可选回退键，校验和注册不会要求所有语言包同步增加新语言名称；因此新增语言只需增加各模块语言包、代码生成目录数据并执行同步脚本。
...
- 内置菜单/字典要求所有受支持语言完整；运行期自定义数据可以缺非主语言并回退主语言。
...
3. 为 103 条内置菜单、10 个字典、35 个字典项和内置系统配置提供人工复核后的所有非主语言 `reviewed` 初始化数据。
...
11. **代码生成器**：所有受支持语言配置、语言包/key、菜单翻译、错误 key、预览缺失检查和正式生成阻断。
12. **版本化迁移**：提交 `v0.0.1` 表结构和完整所有受支持语言初始化数据，不调用在线 provider。
13. **全量检查与运行验收**：静态检查、生成、Go 测试、三个前端 workspace 的 lint/type/build，以及所有受支持语言的关键流程。
...
- 同一账号所有受支持语言下菜单结构、ID、路由、权限完全一致，只有标题变化。
- 同一字典所有受支持语言下 `code/value/tag_type` 一致，只有 `name/label` 变化。
...
- 同一错误所有受支持语言保持相同 `code/reason/message_key`，只改变 `message`。
...
- 103 条内置菜单、10 个字典、35 个字典项具有审核后的所有受支持语言数据。
- `go test ./...`、前端 lint、类型检查和生产构建通过。
- 所有受支持语言在管理端、uni-app、Taro 的运行时验收通过。
