...
- 动态菜单、字典、字典项和系统配置使用四张翻译表，运行时仅读取 `reviewed` 译文并回退中文主字段。
- 后端 `errorsx` 保持冻结的六类 `reason`，静态错误目录和 locale 中间件提供三语 `message`。
- 初始化数据和 `v0.0.1` 迁移保留中文主字段，并提供英语、日语审核译文及翻译状态。
...
主表字段是默认 `zh-CN` 的唯一权威值，四张翻译表只保存非默认 locale。当前只创建 `en-US`、`ja-JP` 行，不创建 `zh-CN` 行，避免默认中文在主表和翻译表之间双写。系统配置名称支持所有配置类型翻译，配置值仅对文本和富文本类型翻译，图片、字典和布尔配置值始终读取主表。
...
2. 不回填 `zh-CN` 翻译行，现有主表字段继续作为默认中文。
3. 为 103 条内置菜单、10 个字典、35 个字典项和内置系统配置提供人工复核后的 `en-US`、`ja-JP` `reviewed` 初始化数据。
...
6. 不把运行时调用 Google V1 写进迁移；迁移中的英语和日语必须在提交前已经生成并审核。
