...
翻译接口默认关闭，通过 `translator.enabled=true`、`I18N_TRANSLATION_DRAFT_ENABLED=true` 或应用 metadata 开启。服务端读取受控源文本，限制目标语言和单条 2000 字节大小，串行调用并使用 `translator.timeout` 配置的 HTTP 超时（默认 8 秒）；已有非空翻译不会被覆盖。系统配置名称以及文本、富文本类型的值会进入机器翻译，图片和布尔值不会外发。
