...
动态菜单、字典、配置等数据库翻译使用 `base_i18n` 的目标键和资源编号，不复用固定文案 key。

## 动态资源翻译

动态资源共用一张翻译表 `base_i18n`，通过字符串 `target_key` 和资源编号标识目标字段：

| `target_key` | 主表字段 |
| --- | --- | --- |
| `base_config.value` | `base_config.value`：文本和富文本配置值 |
| `base_config.name` | `base_config.name`：系统配置名称 |
| `base_dict.name` | `base_dict.name`：字典名称 |
| `base_dict_item.label` | `base_dict_item.label`：字典项标签 |
| `base_menu.meta.title` | `base_menu.meta.title`：菜单标题 |
| `base_job.name` | `base_job.name`：定时任务名称 |
| `base_migration.description` | `base_migration.description`：迁移说明 |
| `base_oauth_provider.name` | `base_oauth_provider.name`：OAuth 登录方式名称 |
| `base_oauth_provider.description` | `base_oauth_provider.description`：OAuth 登录方式提示语 |

`target_key` 使用 `表名.字段名` 格式；嵌套 JSON 字段使用字段路径，例如 `base_menu.meta.title`。`base_dict.code` 和 `base_dict_item.value` 是稳定的业务标识，不进入翻译表。

创建或更新动态资源时，主语言文本写入主表；手动输入的非主语言文本直接写入 `base_i18n`。未输入的语言由翻译任务补充，已有非空翻译不会被覆盖；主语言不在翻译表重复保存，缺失时回退主表。

内置菜单、字典、字典项和系统配置的固定译文位于唯一的 `v0.0.1` 初始化迁移中的 `i18n.<locale>.up.sql`。迁移使用幂等的 `INSERT IGNORE`，不会覆盖已经维护的数据；已执行版本不会因重写文件自动修复，存量数据库应按升级或开发环境重建流程处理，不在本仓库新增后续版本迁移。
